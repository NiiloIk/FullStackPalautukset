...
describe('Favourite blog', () => {
    ...
    test('of empty list returns zero', () => {
        result = favouriteBlog(emptyBlogs)
        expect(result).toBe(0)
    })

    test('multiple blogs return true value', () => {
        result = favouriteBlog(multipleBlogs)
        expect(result).toEqual(multipleBlogs[2])
    })

    test('one blog returns itself', () => {
        result = favouriteBlog(blogWithOneLike)
        expect(result).toEqual(blogWithOneLike[0])
    })
})
