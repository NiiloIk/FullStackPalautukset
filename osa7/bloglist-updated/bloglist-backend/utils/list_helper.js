...
const totalLikes = blogs => {
  return blogs.length === 0
    ? 0
    : blogs.map(blog => blog.likes).reduce((n, sum) => n + sum)
}
...
const mostBlogs = blogs => {
  ...
  blogs.forEach(blog => {
    const existingAuthor = authorsPosts.find(
      post => post.author === blog.author
    )
  ...
  const authorWithMostBlogs = authorsPosts.reduce(
    (max, current) => (current.blogs > max.blogs ? current : max),
    { blogs: -1 }
  ).author
...
const mostLikes = blogs => {
  ...
  const authorWithMostLikes = authorsPosts.reduce(
